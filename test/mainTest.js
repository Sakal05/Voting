...
describe("Contract", function () {
  ...
  before(async function () {
    ...
    const event = contract.filters.ProposalEvent();
    contract.on(event, (id, owner, title, description, monthlyIncentive) => {
      console.log(`Proposal ${id} created by ${id}.`);
      console.log(`Owner is ${owner}.`);
      console.log(`Title: ${title}`);
      console.log(`Description: ${description}`);
      console.log(`URL: ${monthlyIncentive}`);
    });
    ...
    // console.log("Porposal Counter: ", proposalCounter);

    for (let i = 0; i < addresses.length - 2; i++) {
      //purpopse to test fail for add 6 and 7 which doesn't have token allowance
  ...
  describe("Vote on Proposal", function () {
    ...
    describe("Fail Cases; should faild as:", function () {
      it("signer doesn't have enough token to delegate vote right", async function () {
        await expect(
          contract.connect(addresses[5]).delegate(addresses[5].address)
        ).to.revertedWith("Address must have at least 100 tokens");
      });

      it("signer doesn't have enough token to delegate vote right", async function () {
        await expect(
          contract.connect(addresses[5]).vote(0, 0, 100)
        ).to.revertedWith("Insufficient balance");
      });

      it("signer doesn't have enough token allowance", async function () {
        ...
        await expect(
          contract.connect(addresses[5]).vote(0, 0, 100)
        ).to.revertedWith("Token allowance not set");
      });

      it("signer doesn't have vote right", async function () {
        await tokenContract
          .connect(addresses[5])
          .approve(contract.address, BigInt(200) * DECIMAL);
        await expect(
          contract.connect(addresses[5]).vote(0, 0, 100)
        ).to.revertedWith("You have no right to vote!!");
      });

      it("proposal doesn't exist", async function () {
        await contract.connect(addresses[5]).delegate(addresses[5].address);
        await expect(
          contract.connect(addresses[5]).vote(5, 0, 100)
    ...
    describe("Pass cases:", function () {
      ...
      describe("Declare winning proposal", function () {
        ...
        describe("Fail cases as:", function () {
          it("Proposal doesn't exist", async function () {
            await expect(
              contract.connect(owner).declareWinningProposal(4)
            ).to.revertedWith("Proposal does not Exist");
          });

          it("Caller is not the owner of the proposal", async function () {
            await expect(
              contract.connect(addresses[1]).declareWinningProposal(1)
            ).to.revertedWith("You must be the owner of the proposal");
          });

          it("Proposal has already been declared", async function () {
            await expect(
              contract.connect(owner).declareWinningProposal(1)
            ).to.revertedWith("Proposal has already been evaluate");
          });

          it("Proposal has no vote", async function () {
            await expect(
              contract.connect(owner).declareWinningProposal(2)
            ).to.revertedWith("Proposal doesn't have any vote");
          });
        });
        ...
        describe("Pass cases:", function () {
          ...
          describe("Claiming Incentive", function () {
            ...
            describe("Fail cases as:", function () {
              ...
              it("voter already claim everything", async function () {
                await time.increase(86400 * 30 * 8);
              ...
              it("claim proposal that doesn't exist", async function () {
                await time.increase(86400 * 30); //increase by 1 day from proposal declared
                await expect(
                  contract
                    .connect(addresses[1])
                    .claimVotingIncentive(4)
                ).to.revertedWith("Proposal does not Exist");
              });

              it("Caller didn't vote on x proposal", async function () {
                await expect(
                  contract
                    .connect(addresses[5])
                    .claimVotingIncentive(1)
                ).to.revertedWith("You have not voted on this proposal");
              });

              it("Caller voted REJECT on x proposal", async function () {
                await expect(
                  contract
                    .connect(addresses[4])
                    .claimVotingIncentive(1)
                ).to.revertedWith("Voter must vote approve");
              });
            });
